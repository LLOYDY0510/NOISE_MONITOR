...
<head>
  ...
  <title>LibraryQuiet – Live Monitoring (Admin)</title>
  <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800;900&family=JetBrains+Mono:wght@400;500;600;700&display=swap" rel="stylesheet"/>
  <link rel="stylesheet" href="/NOISE_MONITOR/assets/monitoring-admin.css"/>
  <script>window.__LQ_SESSION__ = <?= json_encode($user) ?>;</script>
  <style>
    /* ============================================================
   LibraryQuiet – monitoring-admin.css
   ============================================================ */
...
.sensor-count{display:flex;align-items:center;gap:6px;background:#eff6ff;border:1px solid #bfdbfe;padding:5px 12px;border-radius:20px}
.sc-icon{font-size:13px}.sc-label{font-size:11px;font-weight:600;color:#1e40af}
#sensor-online{font-size:12px;font-weight:800;color:#1d4ed8}
...
/* ADMIN CONTROLS */
.admin-controls{background:#eff6ff;border:1px solid #bfdbfe;border-radius:12px;padding:12px 14px;margin-top:8px}
.ac-label{font-size:10px;font-weight:700;color:#1e40af;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px}
.ac-btns{display:flex;gap:8px;flex-wrap:wrap}
.ac-btn{padding:6px 14px;background:#fff;border:1.5px solid #bfdbfe;border-radius:8px;font-size:12px;font-weight:700;color:#1d4ed8;cursor:pointer;text-decoration:none;font-family:'Plus Jakarta Sans',sans-serif;transition:all .2s}
.ac-btn:hover{background:#dbeafe;border-color:#93c5fd}
...
.sl-note{font-size:11px;font-weight:500;color:#bfdbfe;background:#eff6ff;padding:2px 10px;border-radius:20px;text-transform:none;letter-spacing:0}

/* ZONE GRID */
.zone-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}
.zone-card{background:#fff;border-radius:var(--radius);padding:20px;border:2px solid;box-shadow:0 1px 8px rgba(0,0,0,.06);transition:all .4s ease;cursor:pointer;text-decoration:none;display:block}
.zone-card:hover{transform:translateY(-2px);box-shadow:0 6px 20px rgba(0,0,0,.1)}
...
.zc-manage{font-size:10px;color:#1d4ed8;font-weight:700;background:#eff6ff;padding:3px 8px;border-radius:6px}
...
.btn-sm{padding:6px 14px;background:linear-gradient(135deg,#3b82f6,#1d4ed8);color:#fff;border-radius:8px;font-size:11px;font-weight:700;text-decoration:none;transition:all .2s}
...
.tbl-action{padding:4px 10px;border:none;border-radius:6px;font-size:11px;font-weight:700;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;transition:all .2s}
.ta-config{background:#eff6ff;color:#1d4ed8}.ta-config:hover{background:#dbeafe}

/* MODAL */
.modal-overlay{display:none;position:fixed;inset:0;background:rgba(15,23,42,.55);backdrop-filter:blur(4px);z-index:200;animation:fadeIn .2s}
.modal-overlay.show{display:block}
@keyframes fadeIn{from{opacity:0}to{opacity:1}}
.modal{display:none;position:fixed;top:50%;left:50%;transform:translate(-50%,-48%);width:420px;max-width:95vw;background:#fff;border-radius:20px;box-shadow:0 24px 60px rgba(0,0,0,.2);z-index:300;overflow:hidden;animation:slideUp .3s cubic-bezier(.16,1,.3,1)}
...
.modal-header{padding:22px 24px 18px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:flex-start}
...
.btn-secondary{padding:9px 20px;background:#f1f5f9;border:1.5px solid var(--border);border-radius:10px;color:var(--muted);font-size:13px;font-weight:700;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif}
...
<body>
  ...
  <aside id="sidebar">
    ...
    <nav>
      ...
      <a class="nav-item active" href="/NOISE_MONITOR/dashboards/admin/monitoring-admin.php"><span class="ni">◎</span><span class="nl">Live Monitoring</span></a>
  ...
  <div id="main">
    <header id="topbar">
      ...
      <div class="tb-right">
        ...
        <div class="sensor-count"><span class="sc-icon">📡</span><span id="sensor-online">0/0</span><span class="sc-label">Sensors Online</span></div>
    ...
    <div id="content">
      ...
      <div class="overall-card">
        <div class="ov-left">
          ...
          <div class="admin-controls">
            <div class="ac-label">Admin Controls</div>
            <div class="ac-btns">
              <button class="ac-btn" onclick="silenceAll()">🔕 Silence All</button>
              <a class="ac-btn" href="/NOISE_MONITOR/dashboards/manager/zones.php">▦ Manage Zones</a>
              <a class="ac-btn" href="/NOISE_MONITOR/dashboards/admin/users.php">◉ Users</a>
            </div>
          </div>
        </div>
        <div class="ov-right">
      ...
      <div class="section-label">Zone Readings <span class="sl-note">Admin: click any card to manage zone</span></div>
      ...
      <div class="card" style="margin-top:16px;">
        <div class="card-head-row">
          ...
          <div class="card-head-actions">
            ...
            <a class="btn-sm" href="/NOISE_MONITOR/dashboards/manager/zones.php">Configure Zones</a>
          </div>
        </div>
        <div class="tbl-wrap">
          <table>
            <thead>
              <tr><th>Sensor ID</th><th>Zone</th><th>Floor</th><th>Current Reading</th><th>Status</th><th>Battery</th><th>Last Ping</th><th>Action</th></tr>
            </thead>
  ...
  <!-- SILENCE MODAL -->
  <div class="modal-overlay" id="silence-overlay" onclick="closeSilence()"></div>
  <div class="modal" id="silence-modal">
    <div class="modal-header">
      <div><div class="modal-title">Silence All Alerts</div><div class="modal-sub">Admin action</div></div>
      <button class="modal-close" onclick="closeSilence()">✕</button>
    </div>
    <div class="modal-body" style="text-align:center;padding:28px 24px;">
      <div style="font-size:40px;margin-bottom:12px;">🔕</div>
      <p style="font-size:14px;color:#64748b;line-height:1.6;">This will resolve all <strong id="silence-count">0</strong> active alerts. Are you sure?</p>
    </div>
    <div class="modal-footer">
      <button class="btn-secondary" onclick="closeSilence()">Cancel</button>
      <button class="btn-danger" onclick="confirmSilence()">Yes, Silence All</button>
    </div>
  </div>

  <div class="toast" id="toast"></div>
  <script src="/NOISE_MONITOR/app-data.js"></script>
  <script src="/NOISE_MONITOR/monitoring-admin.js"></script>
</body>
</html>
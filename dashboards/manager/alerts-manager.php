...
<head>
  ...
  <style>
/* ============================================================
   LibraryQuiet – alerts-manager (teal theme) — COMBINED
   ============================================================ */
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
:root{
  --teal:#0d9488;--green:#10b981;--yellow:#f59e0b;--red:#ef4444;--blue2:#3b82f6;
  --bg:#f0f4f8;--sidebar:#0f172a;--sb2:#1e293b;--border:#e2e8f0;
  --text:#0f172a;--muted:#64748b;--light:#94a3b8;--radius:16px
}
...
/* ── BUTTONS ── */
.btn-primary{padding:9px 18px;background:linear-gradient(135deg,#0d9488,#0f766e);border:none;border-radius:10px;color:#fff;font-size:13px;font-weight:700;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;transition:all .2s}
.btn-primary:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(13,148,136,.3)}
.btn-resolve-all{padding:9px 18px;background:#f0fdfa;border:1.5px solid #0d9488;border-radius:10px;color:#0d9488;font-size:13px;font-weight:700;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;transition:all .2s}
.btn-resolve-all:hover{background:#0d9488;color:#fff}
...
.tb-view{background:#f0fdfa;color:#0d9488}
.tb-view:hover{background:#ccfbf1}
.tb-resolve{background:#f0fdf4;color:#15803d}
.tb-resolve:hover{background:#dcfce7}
...
.modal{display:none;position:fixed;top:50%;left:50%;transform:translate(-50%,-48%);width:580px;max-width:95vw;max-height:90vh;background:#fff;border-radius:20px;box-shadow:0 24px 60px rgba(0,0,0,.2);z-index:300;flex-direction:column;overflow:hidden;animation:slideUp .3s cubic-bezier(.16,1,.3,1)}
...
.modal-title{font-weight:800;font-size:16px;color:var(--text)}
.modal-sub{font-size:12px;color:var(--light);margin-top:2px}
...
/* ── TOAST ── */
.toast{position:fixed;bottom:24px;right:24px;padding:12px 20px;border-radius:12px;font-size:13px;font-weight:600;color:#fff;z-index:9999;box-shadow:0 4px 20px rgba(0,0,0,.25);font-family:'Plus Jakarta Sans',sans-serif;transition:all .3s;opacity:0;transform:translateY(10px);pointer-events:none;max-width:340px}
...
<body>
    ...
    <div id="content">
      ...
      <div class="stat-grid">
        <div class="stat-card red">
          <div class="stat-top"><div><div class="stat-label">Active Alerts</div><div class="stat-val" id="s-active">0</div><div class="stat-sub">Needs attention</div></div><div class="stat-icon">🔴</div></div>
        </div>
        <div class="stat-card yellow">
          <div class="stat-top"><div><div class="stat-label">Critical</div><div class="stat-val" id="s-critical">0</div><div class="stat-sub">Critical violations</div></div><div class="stat-icon">⚠️</div></div>
        </div>
        <div class="stat-card green">
          <div class="stat-top"><div><div class="stat-label">Resolved</div><div class="stat-val" id="s-resolved">0</div><div class="stat-sub">Cleared today</div></div><div class="stat-icon">✅</div></div>
        </div>
        <div class="stat-card blue">
          <div class="stat-top"><div><div class="stat-label">Total Logged</div><div class="stat-val" id="s-total">0</div><div class="stat-sub">All records</div></div><div class="stat-icon">📋</div></div>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="filter-bar">
          <div class="search-wrap">
            ...
            <input class="search-input" id="search-input" type="text" placeholder="Search zone or message..." oninput="filterAlerts()"/>
          </div>
          ...
          <select class="filter-select" id="type-filter" onchange="filterAlerts()">
            <option value="">All Types</option>
            <option value="critical">Critical</option>
            <option value="warning">Warning</option>
          </select>
          <select class="filter-select" id="zone-filter" onchange="filterAlerts()">
        ...
        <div class="tbl-wrap">
          <table>
            <thead>
              <tr>
                <th>#</th><th>Date & Time</th><th>Zone</th><th>Noise Level</th>
                <th>Type</th><th>Message</th><th>Status</th><th>Resolved By</th><th>Action</th>
              </tr>
            </thead>
  ...
  <!-- ══════════════════════════════════════════════════════
       COMBINED JAVASCRIPT  (app-data.js + alerts-manager.js)
       ══════════════════════════════════════════════════════ -->
  <script>
  /* ============================================================
     AppData — data layer (formerly app-data.js)
     ============================================================ */
  const AppData = (() => {
  ...
  /* ============================================================
     Page Logic — alerts-manager (formerly alerts-manager.js)
  ...
  function resetFilters() {
    currentTab = 'all'; currentFilters = { search: '', type: '', zone: '' };
    ...
    el('view-body').innerHTML = `
      ...
      <div style="border-top:1px solid #f1f5f9;padding-top:16px;">
        <div style="font-weight:800;font-size:13px;margin-bottom:4px;">
          💬 Notes & Messages
          <span style="background:#f1f5f9;color:#64748b;font-size:11px;padding:2px 8px;border-radius:20px;font-weight:600;">${msgs.length}</span>
        </div>
        ...
        <div id="msg-thread" style="max-height:200px;overflow-y:auto;padding:4px 0;margin-bottom:12px;">
          ${msgs.length === 0
            ? '<div style="text-align:center;color:#94a3b8;font-size:12px;padding:20px;">No notes yet.</div>'
            : msgs.map(m => bubbleHtml(m, session)).join('')}
        </div>
        <div style="display:flex;gap:8px;align-items:flex-end;">
          <textarea id="msg-input" placeholder="Leave a note..." rows="2"
            style="flex:1;padding:10px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:13px;font-family:'Plus Jakarta Sans',sans-serif;resize:none;outline:none;"
            onfocus="this.style.borderColor='#0d9488'" onblur="this.style.borderColor='#e2e8f0'"></textarea>
          <button onclick="sendMessage('${id}')"
            style="padding:10px 16px;background:#0d9488;color:#fff;border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;white-space:nowrap;">
            📨 Send
          </button>
        </div>
      </div>`;
  ...
  async function sendMessage(alertId) {
    ...
    await AppData.addAlertMessage(alertId, {
      from: session?.name || 'Library Manager',
